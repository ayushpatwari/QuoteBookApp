...
final class QuoteViewModel: ObservableObject {
    ...
    func addQuoteToLiked(quote: DiscoverQuote)
    {
        if let user = Auth.auth().currentUser?.uid
        {
            let db = Firestore.firestore()
            
            db.collection("users").document(user).collection("collections").document("liked").collection("quotes").addDocument(data: ["quote_id": quote.id ?? "N/A"])
        }
    }
